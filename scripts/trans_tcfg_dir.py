...
def main():
    ...
    logger.log(
        f"dir: {args.img_dir} batch_size: {batch_size} class_cond: {args.class_cond}"
    )
    logger.log("================================================================")
    ...
    # ================================================================
    # load image
    logger.log(f"open image... path: {args.img_dir}")
    # -- test --------------------------------------------------------
    ...
    for i_batch, img_path_batch in enumerate(batch_iterator(img_list, batch_size)):
        ...
        label_to = th.tensor([int(args.label_to)] * batch_size).to(device)
        label_from = th.tensor([int(args.label_from)] * batch_size).to(device)
        sample_fn = diffusion.tcfg_sample_loop
