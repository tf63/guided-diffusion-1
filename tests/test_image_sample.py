...
def main():
    ...
    x = th.randn((batch_size, 3, 64, 64)).to(device)
    for i in tqdm(reversed(range(1, 1000))):
        ti = (th.ones(batch_size) * i).long().to(device)

        predicted_noise = diffusion.diffuser_wrap(model, x, ti)
